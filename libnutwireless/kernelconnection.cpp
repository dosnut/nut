...
//wireless extension specific
	...
	void CkernelConnection::tryScanResults() {
	...
	void CKernelConnection::readWirelessInfo() {
	...
	void CKernelConnection::parseWextIeWpa(unsigned char * iebuf, int buflen, WextRawScan * scan) {
...
//public:
	CKernelConnection::CKernelConnection(QObject * parent, QString * deviceName)
	: QObject(parent), m_deviceName(deviceName), m_signalQualityPollRate(5000) {
	}
	
	CKernelConnection::~CKernelConnection() {
		disconnectFromKernel();
	}
	
	bool CKernelConnection::connectToKernel();
	bool CKernelConnection::disconnectFromKernel();
	
	void CKernelConnection::setSignalQualityPollRate(int msec) {
		m_signalQualityTimeoutCount = 0;
		if (m_signalQualityTimerId != -1) {
			qDebug() << "Setting signal quality polling (restarting timer)";
			killTimer(m_signalQualityTimerId);
			m_signalQualityPollRate = msec;
			m_signalQualityTimerId = startTimer(m_signalQualityPollRate);
		}
		else  {
	...
	int CKernelConnection::getSignalQualityPollRate() {
		return m_signalQualityPollRate;
	}
	
	WextSignal CKernelConnection::getSignalQuality() {
		return m_signalQuality;
	}
	
	QList<ScanResult> CKernelConnection::getScanResults() {
		return m_scanResults;
	}

//slots
	void CKernelConnection::scan();
