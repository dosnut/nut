...
namespace qnut {
	...
	void COverViewModel::deviceAdded(CDevice * device) {
		...
		if (device->type == DT_AIR)
			connect(device->wpa_supplicant, SIGNAL(signalQualityUpdated(libnutws::wps_wext_signal_readable)), this, SIGNAL(layoutChanged()));
		emit layoutChanged();
	}
	...
	void COverViewModel::deviceRemoved(CDevice * device) {
		...
		if (device->type == DT_AIR)
			disconnect(device->wpa_supplicant, SIGNAL(signalQualityUpdated(libnutws::wps_wext_signal_readable)), this, SIGNAL(layoutChanged()));
		emit layoutChanged();
	}
